  ...
  <section>
    <div  :style="pageStyle">
      ...
      <el-row style="margin-left: 20px; margin-right: 20px;">
        ...
        <!-- <x-object-list-view ref="objectAttr" :row-edit="true" :object-id="" :view-json="objectAttrViewJson">
...
import { callObjectAction } from '@/api/object-data'

export default {
  name: 'cs-cost-object-config',
  ...
  data() {
    return {
      ...
      //
      objectAttrObjectId: null,
      objectAttrViewId: null,
      objectAttrViewJson: null
    }
  },
  mounted() {
    this.autoFixAndShow()
  ...
  methods: {
    ...
    // autoFixAndShow() {
    //   callObjectAction(this.objectId, this.objectDataId, 'autoUpdateAttr', null).then(ret => {
    //     if (ret.success) {
    //       this.showObjectAttr()
    //     }
    //   })
    ...
    showObjectAttr() {
      ...
      getViewDefineById(this.objectAttrViewId).then(ret => {
        if (ret.success) {
          this.objectAttrViewJson = JSON.parse(ret.data.viewContent)
        }
      })
    }
  }
